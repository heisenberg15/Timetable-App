...
public class SatFragment extends Fragment
{
    ...
    {
        View fragmentView = inflater.inflate(R.layout.fragment_saturday, container, false);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState)
    {
        ...
        recyclerView = getActivity().findViewById(R.id.sat_recycler_view_id);
    ...
    public void createSubject(SubjectModel subjectModel)
    {
        if (mainActivity.edit == 0) {
            Singleton.getInstance().addSatSubject(subjectModel);

            RecyclerAdapter recyclerAdapter = new RecyclerAdapter(getContext(), Singleton.getInstance().getSatList());
            recyclerView.setAdapter(recyclerAdapter);
        } else if (mainActivity.edit ==1){
            Singleton.getInstance().getSatList().set(mainActivity.position, subjectModel);
    ...
    private void updateSubjectList()
    {
        ...
        if (saveSingleton.getString("saturdayList", null) != null) {
            RecyclerAdapter recyclerAdapter = new RecyclerAdapter(getContext(), Singleton.getInstance().getSatList());
            recyclerView.setAdapter(recyclerAdapter);
        }
    }
